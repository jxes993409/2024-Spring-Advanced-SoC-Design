...
`ifdef USER_PROJECT_SIDEBAND_SUPPORT
	localparam	FIFO_WIDTH = (pUSER_PROJECT_SIDEBAND_WIDTH + 4 + 4); //upsb, tstrb, tkeep
`else
	localparam	FIFO_WIDTH = (4 + 4);		//tstrb, tkeep
`endif
...
always @(posedge axis_clk or negedge axis_rst_n)  begin
  if (!axis_rst_n) begin
    ...
    if (ss_tready && ss_tvalid) begin
      `ifdef USER_PROJECT_SIDEBAND_SUPPORT
        fifo[w_ptr] <= {ss_tupsb, ss_tstrb, ss_tkeep}; 
      `else
        fifo[w_ptr] <= {ss_tstrb, ss_tkeep}; 
      `endif
      w_ptr <= w_ptr + 1;
    end
  end
...
`ifdef USER_PROJECT_SIDEBAND_SUPPORT
	assign {sm_tupsb, sm_tstrb, sm_tkeep} = fifo[r_ptr];
`else
	assign {sm_tstrb, sm_tkeep} = fifo[r_ptr];
`endif
